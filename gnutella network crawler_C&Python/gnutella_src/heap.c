...
struct heap
{
		...
		//offset 用在當 void * root[i] 並非 heap_loc_t 時, 而是一個把 heap_loc_t 包起來的
		//struct, 用 offset 指向 heap_loc_t 所在的位置
        int offset;  //!< offset into void * to find a heap_loc_t
        void **root; // 一維的 void * 陣列
        int max_n;
        int n;
};
...
// 取出 index 為 heap->offset 的 element 的位址(&(((char *)...), 接著轉型成 heap_loc_t * 取值
// (*(heap_loc_t *))
